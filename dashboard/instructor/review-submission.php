...
// 2. Fetch Questions + Answers + Resolve Option Text for MCQs
$quiz_audit = [];
if ($sub['quiz_mode'] === 'digital') {
    ...
    foreach ($raw_audit as $item) {
        $resolved_response = $item['student_raw_response'];
        
        // Resolve MCQ Index to actual text
        if ($item['type'] === 'multiple_choice' && $item['options'] && $resolved_response !== '') {
            $opts = json_decode($item['options'], true);
            $resolved_response = $opts[$resolved_response] ?? $resolved_response;
        }

        $quiz_audit[] = array_merge($item, ['resolved_response' => $resolved_response]);
    }
}
...
<style>
    ...
    html { background: #f8fafc; transition: background-color 0.3s ease; }
...
function reviewManager(auditData, maxPoints) {
    return {
        ...
        questions: auditData.map(q => ({
            ...q,
            // Pre-fill MCQ scores if they are correct
            awarded_points: (q.type !== 'short_answer' && String(q.student_raw_response) === String(q.correct_answer)) ? q.max_q_points : 0
        })),
        ...
        async submitGrade() {
            ...
            formData.append('score', this.percent); // We save the calculated percentage
