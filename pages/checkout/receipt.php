 ...
 <div class="container text-center">
  <div class="card shadow-lg p-5 mx-auto" style="max-width: 600px;">
   ...
   <h1 class="display-5 fw-bold mb-3">Enrollment Successful!</h1>
      ...
      <p class="lead text-muted">Congratulations, <strong><?= htmlspecialchars($enrollment['first_name']) ?></strong>! Your payment has been confirmed and you are now enrolled in the following course.</p>
   
   <hr class="my-4">
   
   <h4 class="fw-bold text-primary mb-2"><?= htmlspecialchars($enrollment['title']) ?></h4>
   
      <ul class="list-unstyled mb-4 small text-muted text-start mx-auto" style="max-width: 300px;">
    <li><strong>Amount Paid:</strong> ₵<?= number_format($enrollment['amount_paid'], 2) ?></li>
    <li><strong>Transaction Ref:</strong> <?= htmlspecialchars($enrollment['transaction_ref']) ?></li> 
    <li><strong>Date:</strong> <?= date('F j, Y', strtotime($enrollment['enrolled_at'])) ?></li>
   </ul>
