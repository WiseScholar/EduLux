...
// === QUICK STATS: REFINED COURSE COUNTS ===

$total_students = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'student'")->fetchColumn();
$total_instructors = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'instructor'")->fetchColumn();

// 1. Course Status Counts
$live_courses_count = $pdo->query("SELECT COUNT(*) FROM courses WHERE status = 'published'")->fetchColumn(); 
$pending_courses_count = $pdo->query("SELECT COUNT(*) FROM courses WHERE status = 'pending'")->fetchColumn(); 

// Pending Instructor count (Kept for the revenue/activity widget below)
$pending_instructors_count = $pdo->query("SELECT COUNT(*) FROM users WHERE role = 'instructor' AND approval_status != 'approved'")->fetchColumn();
    ...
    <h2 class="mb-4 fw-bold">Welcome back, <?php echo htmlspecialchars($_SESSION['first_name'] ?? 'Admin'); ?>!</h2>
        ...
        <div class="stat-card d-flex align-items-center">
          ...
          <div>
            ...
            <div class="col-lg-3 col-md-6">
        <div class="stat-card d-flex align-items-center">
          <div class="stat-icon text-warning me-4"><i class="fas fa-book-open"></i></div>
          <div>
            <h3 class="mb-0"><?php echo number_format($live_courses_count); ?></h3>
        ...
        <div class="stat-card d-flex align-items-center">
          ...
          <div>
            <h3 class="mb-0 text-danger"><?php echo number_format($pending_courses_count); ?></h3>
            <small class="text-muted">Pending Course Reviews</small>
          </div>
        </div>
      </div>
    </div>
