...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['primary_video']) && $_FILES['primary_video']['error'] !== UPLOAD_ERR_NO_FILE && validate_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if (in_array($ext, $allowed) && $file['size'] <= 500_000_000) {
        ...
        if (move_uploaded_file($file['tmp_name'], $destination)) {
            ...
            $msg = "Primary video uploaded successfully!";
        } else {
...
// 2. Auto-save title + preview only
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_FILES['primary_video']) && validate_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    $pdo->prepare("UPDATE course_lessons SET title=?, is_free_preview=? WHERE id = ?")
        ->execute([$title, $is_preview, $lesson_id]);
...
<head>
    ...
    <style>
        ...
        .bg-gradient-primary { background: linear-gradient(135deg, rgba(99,102,241,0.3) 0%, rgba(139,92,246,0.2) 100%); }
    </style>
</head>
<body class="instructor-layout">
    ...
    <div class="main-content">
        ...
        <!-- Messages -->
        <?php if (!empty($msg)): ?>
            <div class="alert alert-success mx-4 mt-3 alert-dismissible fade show">
                <?= htmlspecialchars($msg) ?>
                <button type="button" class="btn-close text-white" data-bs-dismiss="alert"></button>
            </div>
        <?php endif; ?>
        <?php if ($video_upload_error): ?>
            <div class="alert alert-danger mx-4 mt-3 alert-dismissible fade show">
                ...
                <div class="d-flex">
                    ...
                    <div>
                        ...
                        <p class="mb-0 text-white-50">
                            Use the <strong>Primary Video</strong> section for the main video students watch.<br>
                            Bonus videos, images, PDFs → upload in <strong>Additional Materials</strong>.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <!-- Full-width Sidebar (now centered) -->
        <div class="editor-main justify-content-center">
            <div class="editor-sidebar">
                <h4 class="mb-4">Lesson Settings</h4>
                
                <div class="mb-5">
                    <label class="form-label text-white fw-bold">Lesson Title</label>
                    <input type="text" id="lessonTitle" class="form-control form-control-lg bg-dark text-white border-0" 
                           value="<?= htmlspecialchars($lesson['title']) ?>">
                </div>

                <?php if ($lesson['type'] === 'video'): ?>
                    <div class="mb-5 p-4 bg-gradient-primary rounded-3 border border-primary">
                        <label class="form-label text-white fw-bold fs-5 mb-3">
                            <i class="fas fa-play-circle me-2"></i> Primary Video (Triggers Completion)
                        </label>

                        <div class="mb-4">
                            <label class="form-label text-white">Upload Video File (MP4, WebM, MOV - max 500MB)</label>
                            <form method="POST" enctype="multipart/form-data">
                                <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                                <input type="file" name="primary_video" class="form-control bg-dark text-white mb-3" accept="video/*">
                                <button type="submit" class="btn btn-primary w-100">Upload Primary Video</button>
                            </form>
                        </div>

                        <div class="mb-3">
                            <label class="form-label text-white">OR External Link (YouTube/Vimeo)</label>
                            <input type="url" id="videoUrl" class="form-control bg-dark text-white" 
                                   value="<?= htmlspecialchars($lesson['video_url'] ?? '') ?>" 
                                   placeholder="https://youtu.be/...">
                        </div>

                        <small class="text-white-50">
                            Only <strong>one primary video</strong> — this is what students watch to complete the lesson.
                        </small>
                    </div>
                <?php endif; ?>

                <h5 class="text-white fw-bold mb-3">
                    <i class="fas fa-folder-plus me-2"></i> Additional Materials
                </h5>
                <p class="text-white-50 mb-4">
                    Upload bonus videos, images, PDFs, code files, worksheets, etc.<br>
                    These are <strong>downloadable only</strong> — do not affect completion.
                </p>
                    ...
                    <?php foreach ($materials as $mat): ?>
                        <div class="bg-secondary bg-opacity-20 rounded p-3 mb-2 d-flex justify-content-between align-items-center">
                            ...
                            <a href="<?= BASE_URL ?>assets/uploads/courses/materials/<?= $mat['file_path'] ?>" 
                               target="_blank" class="btn btn-sm btn-outline-light">View</a>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
        ...
        FilePond.create(document.querySelector('.filepond'), {
            server: {
                process: (fieldName, file, metadata, load, error) => {
                    ...
                    .then(r => r.json())
                    .then(data => {
                        if (data.status === 'success') load(data.filename);
