...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validate_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'add') {
        ...
        // IMPORTANT: Prevent deletion if courses are linked, or reassign them first.
        // For now, we assume a database constraint handles this or we reassign (for simplicity, we delete).
        
        // Find existing courses and set their category_id to NULL or a default one (e.g., ID 1)
        // Reassignment is safer, but complex. We'll simply disallow if courses exist for now.
