...
// --- FIX: Update Submission Logic to handle 'published' as re-submission to 'pending' ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validate_csrf_token($_POST['csrf_token'] ?? '') && $_POST['action'] === 'submit') {
    
    $old_status = $course['status'];
    $new_status = 'pending'; // Always send course to pending status for admin review

    // Check if the current status allows submission (Draft, Published, or Rejected)
    if ($old_status === 'draft' || $old_status === 'published' || $old_status === 'rejected') {
        
        $pdo->prepare("UPDATE courses SET status=?, submitted_at=NOW() WHERE id=?")->execute([$new_status, $course_id]);
            
        $course['status'] = $new_status; // Update variable for immediate display
        
        if ($old_status === 'published') {
            $msg = "Success! Your course updates have been submitted for admin re-review.";
        } else {
    ...
    <div class="container-fluid">
      ...
      <div class="publish-card">
        ...
        <div class="text-center mt-5">
          <?php 
                        $can_submit = ($course['status'] === 'draft' || $course['status'] === 'published' || $course['status'] === 'rejected');
          ...
          <?php if ($can_submit): ?>
                        <div class="mb-5">
                            <p class="lead text-muted">
                                By clicking <strong>Submit for Review</strong>, you confirm that your course is complete and ready for admin processing.
                            </p>
                            <?php if ($is_published): ?>
                                <p class="text-warning fs-5 fw-bold">
                                    <i class="fas fa-exclamation-triangle me-2"></i> Warning: This will temporarily change your course status to PENDING for re-review.
                                </p>
                            <?php else: ?>
                        ...
                        <form method="POST" class="d-inline">
                            ...
                            <button type="submit" class="final-btn text-white">
                                <i class="fas fa-upload me-2"></i> Submit for Admin Review
                            </button>
                        </form>
          ...
          <?php endif; ?>
            ...
            <div class="alert alert-success text-center py-5 mt-4">
              <i class="fas fa-check-circle fa-4x mb-4 text-success"></i>
              <h3>Course is Currently LIVE!</h3>
              <a href="<?= BASE_URL ?>pages/courses/detail.php?id=<?= $course_id ?>" target="_blank" class="btn btn-light btn-lg">
                View Live Course
              </a>
            </div>
          <?php endif; ?>
