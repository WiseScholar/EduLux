<?php
// student/index.php - MAINTAINING ORIGINAL STYLES.CSS AND INLINE BLOCK
...
// Fetch stats (PHP logic remains the same)
$stmt = $pdo->prepare("
 ...
 WHERE e.user_id = ? AND c.is_published = 1 AND e.status = 'completed'
");
...
// Enrolled courses (FIX: Added e.status = 'completed')
$courses_stmt = $pdo->prepare("
  SELECT c.id, c.title, c.thumbnail, c.price, 
       u.first_name, u.last_name, u.avatar as instructor_avatar,
       -- FIX: Use COALESCE to ensure a value is returned for progress and last_accessed
...
// Includes the standard header, which contains global styles and Bootstrap CSS
  ...
  <div class="row g-4 mb-5">
    ...
    <?php elseif ($enrolled > 0): ?> 
      <div class="col-12 text-center py-5">
        <h4 class="text-muted">All your enrolled courses are at 0% progress.</h4>
        <p class="text-muted">Visit **<a href="<?= BASE_URL ?>dashboard/student/my-courses.php" class="text-primary">My Courses</a>** to start learning!</p>
      </div>
    <?php else: ?>
      <div class="col-12 text-center py-5">
...
<script>
    // Light/Dark Mode Toggle (JS logic remains the same)
