                        ...
                        <div class="bg-white dark:bg-slate-800 p-8 lg:p-12 rounded-[2.5rem] border border-slate-100 dark:border-slate-700/50 shadow-sm">
                            <div class="space-y-10">
                                ...
                                <div x-show="form.assignment_mode === 'document'" x-transition x-cloak class="pt-10 border-t border-slate-50 dark:border-slate-700/50">
                                    <div class="bg-indigo-50/30 dark:bg-indigo-900/10 rounded-[2.5rem] p-8 border-2 border-dashed border-indigo-100 dark:border-indigo-900/30">
                                        <div class="flex items-center gap-4 mb-6">
                                            <div class="w-12 h-12 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm">
                                                <i class="fas fa-file-invoice text-xl"></i>
                                            </div>
                                            <h3 class="text-sm font-black uppercase italic tracking-tight dark:text-white">Primary Assessment Brief</h3>
                                        </div>
                                        <p class="text-xs text-slate-500 mb-8 leading-relaxed">In Document Mode, the system prioritizes the first file in your registry as the main assignment paper for students to download.</p>
                                    </div>
                                </div>
                                ...
                                <div class="pt-10 border-t border-slate-50 dark:border-slate-700/50">
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
                                        <?php foreach ($existing_resources as $index => $res): ?>
                                            <div class="flex items-center justify-between bg-white dark:bg-slate-900 p-4 rounded-2xl border <?= ($index === 0 && $assessment['quiz_mode'] === 'document') ? 'border-indigo-500 dark:border-indigo-600 ring-4 ring-indigo-500/5' : 'border-slate-100 dark:border-slate-700' ?> shadow-sm group transition-all">
                                                <div class="flex items-center gap-3 overflow-hidden">
                                                    <i class="fas fa-file-pdf <?= ($index === 0 && $assessment['quiz_mode'] === 'document') ? 'text-indigo-600' : 'text-slate-400' ?>"></i>
                                                    <div>
                                                        <p class="text-[10px] font-bold text-slate-600 dark:text-slate-300 truncate w-32"><?= htmlspecialchars($res['file_name']) ?></p>
                                                        <?php if ($index === 0 && $assessment['quiz_mode'] === 'document'): ?>
                                                            <span class="text-[8px] font-black uppercase text-indigo-500 tracking-widest">Master File</span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                                <button type="button" @click="deleteResource(<?= $res['id'] ?>)" class="text-slate-300 hover:text-red-500 transition-colors"><i class="fas fa-trash-alt"></i></button>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                    ...
                                    <div class="relative group">
                                        ...
                                        <div class="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-[2rem] p-10 text-center bg-slate-50/50 dark:bg-slate-900/50 group-hover:border-indigo-400 transition-all">
                                            <i class="fas fa-upload text-2xl text-slate-300 mb-4"></i>
                                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-500">Upload additional materials</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    function editAssessmentApp() {
        return {
            ...
            form: {
                ...
                assignment_mode: '<?= ($assessment['quiz_mode'] === 'document') ? 'document' : 'standard' ?>',
