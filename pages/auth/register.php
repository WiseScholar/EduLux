            ...
            try {
                ...
                if ($stmt->fetch()) {
                    ...
                    if (empty($errors)) {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO users (username, email, first_name, middle_name, last_name, password_hash, role, approval_status, avatar, verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)");
                        $stmt->execute([$username, $email, $first_name, $middle_name, $last_name, $password_hash, $role, $approval_status, $avatar_path]);
                        $user_id = $pdo->lastInsertId();

                        // Role-specific uploads & details
                        if ($role === 'student') {
                        ...
                        if ($role === 'instructor') {
                            ...
                            if (empty($errors)) {
                                $stmt = $pdo->prepare("INSERT INTO instructor_details (user_id, date_of_birth, gender, nationality, residential_address, mobile_number, linkedin_url, emergency_contact_name, emergency_contact_relationship, emergency_contact_phone, highest_qualification, professional_certifications, areas_of_specialization, modules_qualified, teaching_experience_years, preferred_teaching_format, availability_schedule, credentials_upload, consent_qr_code, consent_cpd_standards, consent_recording) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                                $stmt->execute([
                                    $user_id,
